    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            */
            ...
            fixed4 frag(vOUT i) : SV_Target
            {
                ...
                specAmt = pow(specAmt, 4.0); // 스펙큘러 값을 광택값(shininess) 만큼 거듭제곱해서 스펙큘러의 영역을 확 좁혀서 좀 더 금속같은(?) 쨍한 느낌을 줬었지?
        ...
        Pass
        {
            /*
                ...
                멀티패스는 말 그대로 여러 개의 패스를 
                알파블렌드로 섞어주는 것이므로, 두 번째 패스부터는 생성할 때, 
                Tags 블록에서 라이트모드를 "ForwardAdd" 로 설정해줘야 함.

                첫 번째 패스의 라이트모드는 "ForwardBase" 로 설정했었지?

                여기서 ForwardBase 는 멀티패스 렌더링에서
                가장 먼저 렌더링되는, 기반이 되는 베이스 패스를
                지정하는 것이고,

                ForwardAdd 는 멀티패스 렌더링에서
                베이스 패스 위에 그려서
                알파블렌딩(특히 가산 블렌딩 모드)로
                '가산(add)' 해주는 패스임을 지정하는 것!
            */
            Tags {"LightMode"="ForwardAdd" "Queue"="Geometry"}
            
            Blend One One // 12장에서 했던 것처럼, 가산 블렌드 모드로 패스들을 블렌딩해줘야 하는데, 이때 유니티에서 사용하는 블렌딩 공식이 'One One' 임. (백 버퍼의 색상과 새로운 패스의 프래그먼트 색상을 그냥 더해주는 거임.)
            
            CGPROGRAM
            #pragma vertex vert
            #pragma fragment frag
            #include "UnityCG.cginc"
            #include "AutoLight.cginc" // 새로운 유니티 내장파일을 하나 더 불러옴. (멀티라이트 구현 시 여러 유형의 라이트를 쉽게 다룰 수 있도록 도와주는 거라고 함...)

            // 해당 패스 블록에서 지정했듯이, 이 패스를 라이팅 시스템에서 
            // ForwardAdd 패스로 사용할 수 있도록 유니티 쉐이더 컴파일러에게 요청하는 pragma 문 
            #pragma multi_compile_fwdadd


            // 버텍스 셰이더 함수에서 입력받는 구조체. 베이스 패스랑 동일.
            struct vIN
            {
                float4 vertex : POSITION; // 버텍스 위치 데이터
                float3 normal : NORMAL; // 버텍스 노말 벡터 데이터
            ...
            // 버텍스 셰이더 함수에서 보간되어 다음 파이프라인들로 출력되는 구조체. 
            // 베이스 패스와 거의 동일하나, 라이트 감쇄 계산에 필요한 유니티 매크로를 추가로 정의함. 
            struct vOUT
            {
                float4 pos : SV_POSITION; // 단색 셰이더에서 지정해줬듯이 버텍스 위치 데이터가 보간되어 넘어가는 변수
                float3x3 tbn : TEXCOORD0;
                ...
                /*
                    라이트 감쇄 계산을 편하게 하기 위해
                    선언해줘야 하는 매크로!

                    이 매크로는 내부적으로 두 개의 변수를 사용하는데,
                    이 두 개의 변수를 담기 위해 2개의 float4 타입의
                    TEXCOORD 시맨틱 좌표 세트를 가져다 쓴다고 함.

                    그래서 TEXCOORD5, TEXCOORD6 
                    슬롯을 가져다 쓰도록 하기 위해
                    인자로 5, 6을 넘겨준 것.
                */
                LIGHTING_COORDS(5,6)
            ...
            // 버텍스 함수 (베이스 패스와 거의 동일하나, LIGHTING_COORD 매크로 내부 변수에 값을 쓰도록 새로운 매크로를 버텍스 함수 내부에서 추가로 사용함!)
            vOUT vert(vIN v)
            {
                vOUT o; // 버텍스 출력 구조체를 o 라는 이름의 변수로 가져옴.
                o.pos = UnityObjectToClipPos(v.vertex); // 버텍스 위치좌표에 MVP 행렬을 곱해 클립공간 좌표계로 변환해주는 유니티 내장함수 사용
                ...
                TRANSFER_VERTEX_TO_FRAGMENT(o); // TRANSFER_VERTEX_TO_FRAGMENT() 매크로는 인자로 버텍스 출력구조체 vOUT 을 전달해서 LIGHTING_COORD 내부의 두 변수에 어떤 값을 써주는 역할을 한다고 함.
                return o; // 출력 구조체의 변수들을 모두 계산하여 다음 파이프라인으로 전달함. (프래그먼트 함수에서 보간되어 전달될거임.)
            }
            ...
            // 프래그먼트 함수 (베이스 패스와 거의 유사하나, 감쇄값(falloff)를 계산하기 위해 새로운 매크로를 내부에서 추가로 사용함!)
            fixed4 frag(vOUT i) : SV_Target
            {
                // 조명계산에 필요한 일반벡터들을 먼저 구함
                float3 unpackNormal = UnpackNormal(tex2D(_Normal, i.uv)); // 노말맵에서 텍셀값을 샘플링해온 뒤, -1 ~ 1 사이의 탄젠트공간 노멀벡터로 맵핑해주는 유니티 내장함수 UnpackNormal() 사용.
                ...
                // 조명 감쇄값 계산
                /*
                    LIGHT_ATTENUATION() 매크로를 사용하여
                    조명 감쇄값을 자동으로 계산해 줌.

                    원래 glsl 에서는 조명 위치에서 프래그먼트 거리값과 조명반경
                    이런 값들로 감쇄를 직접 계산해줬지만, 유니티 매크로가 
                    이런 계산을 자동으로 처리해준다는 뜻임!

                    인자로는 LIGHTING_COORDS 매크로가 들어있는
                    vOUT 버텍스 출력 구조체를 넣어줌.

                    아마 버텍스 함수에서 TRANSFER_VERTEX_TO_FRAGMENT 매크로가
                    LIGHTING_COORDS 매크로 내의 두 변수에 써준 값을
                    가져와서 감쇄를 계산하겠지?

                    이렇게 유니티 매크로로 감쇄를 자동 계산해주면
                    감쇄 계산이 필요한 라이트 유형 (스포트라이트, 포인트라이트)를
                    별도의 패스를 만들 필요 없이 동일한 코드로 처리할 수 있는 장점이 있다고 함.
                */
                float falloff = LIGHT_ATTENUATION(i);

                // 라이팅 계산
                float diffAmt = max(dot(nrm, _WorldSpaceLightPos0.xyz), 0.0) * falloff; // 디퓨즈 라이팅 값에 매크로로 계산한 조명 감쇄값을 곱해줌.
                float specAmt = max(dot(halfVec, nrm), 0.0);
                specAmt = pow(specAmt, 4.0) * falloff; // 스펙큘러 라이팅 값에 매크로로 계산한 조명 감쇄값을 곱해줌.
